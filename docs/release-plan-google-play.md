...
| R5-0a | **Новий (виняток 2026-09-01).** COL-4 повністю: `FieldValue.increment()` для HP/slots/ресурсів, `arrayUnion`/`arrayRemove` для conditions — `docs/collaborative-editing.md` §3.1 | Одночасні дельти двох клієнтів на HP/ресурсі не конфліктують, фінальне значення коректне на обох; тести на двох клієнтах |
| R5-0b | **Новий (виняток 2026-09-01).** COL-6+COL-9: presence з heartbeat + `changeHistory[]` → підколекція `characterSheets/{id}/changes` — `docs/collaborative-editing.md` §3.1 | Presence видно на другому клієнті; журнал пишеться в підколекцію, документ персонажа більше не росте від історії; тести на двох клієнтах |
