...
| #    | Задача                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Файли                                                                                                          | DoD                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R1-1 | ✅ **14-денний відлік завершено 2026-08-26 (старт 2026-08-12, підтверджено власником продукту).** Play Console: реєстрація, верифікація, тип акаунта, створення застосунку — ✅. Перший AAB в Internal testing — ✅. Випуск `1.0.0 (4) — закритий тест` схвалено Google, усі три пункти чекліста «Робоча версія» виконано: ~~«Опублікувати версію для закритого тестування»~~ ✅, ~~«Залучити принаймні 12 користувачів»~~ ✅, ~~«Провести закрите тестування тривалістю принаймні 14 днів зі щонайменше 12 користувачами»~~ ✅. Кнопка «Подати заявку» стала активною, **заявку на доступ до робочої версії подано 2026-08-27 (четвер, 20:24)** | —                                                                                                              | 12 тестувальників набрано, реліз схвалено, 14-денний відлік завершено, заявка на production access подана 2026-08-27 — очікуємо рішення Google (типово до 7 днів, орієнтовно до 2026-09-03), перевірено скріном «Інформаційна панель» 2026-08-28, див. `docs/release-blockers.md`                                                                                                                                                                                                                                                                                                                                                                                        |
...
| R1-4 | **Видалення акаунта** (PLY-1 + COL-10 + SEC-5): екран у Settings, re-auth, callable Cloud Function із каскадом owner/editor і передачею ownership, правило `delete` для `users`, веб-сторінка запиту — ✅ **код зроблено (2026-07-26)**, деплой пройшов ланцюжок Blaze/IAM-фіксів 2026-08-12 (`docs/release-blockers.md` п.9), **✅ підтверджено робочим на реальному пристрої власником продукту 2026-08-13**                                                                                                                                                                                                                                        | `Settings.tsx`, `src/services/accountDeletion.ts`, `functions/src/deleteMyAccount.ts`, `firestore.rules`, i18n | Акаунт видаляється; **аркуш зі співредакторами не зникає в них**; ownership передано; повторний логін дає чистий профіль; URL живий. **Не перевірено окремо:** створення інвайту/приєднання по коду (Campaign Management, той самий деплой-шлях)                                                                                                                                                                                                                                                                                                                                                                        |
...
| #     | Задача                                                                                                                                                                                                                                                                                                                                                                                                                                                              | DoD                                                                                                                                                                                                    |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| R3-1  | ✅ **Зроблено.** **Зміряти холодний старт** на реальному mid-range Android у release-білді (`performance.now()` + logcat). Спочатку цифра                                                                                                                                                                                                                                                                                                                                            | Є базове число TTI                                                                                                                                                                                     |
...
| R3-7  | ✅ **Зроблено 2026-08-14.** `minifyEnabled` + `shrinkResources` + ProGuard-правила (PERF-2) — обидва прапорці ввімкнено окремими комітами, keep-правила додано лише для бібліотек без власних `consumerProguardFiles` (перевірено кожну в `node_modules`). Два локальні `./gradlew :app:bundleRelease` — BUILD SUCCESSFUL, підтверджено по `mapping.txt`/`configuration.txt`. Деталі — `docs/audit-2026-07.md` PERF-2                                               | **Повний smoke-тест release-білда на реальному пристрої — ще не пройдено** (не зроблено в середовищі без пристрою/емулятора), чекліст §4 нижче                                                         |
...
| R3-10 | ✅ **Зроблено 2026-08-27.** Подати заявку на production access (14-денне вікно завершилось 2026-08-26)                                                                                                                                                                                                                                                                                                                                                                                                       | Заявка відправлена 2026-08-27 (четвер, 20:24); дата зафіксована — скрін «Інформаційна панель» Play Console, перевірено 2026-08-28                                                                                                                                                                   |
...
**Статус (2026-09-01): відмову активно усуваємо, новий 14-денний тест іде.**
Google відповів 2026-08-31 (03:14) — «more testing required», причини:
тестувальники недостатньо залучені + не показано збору/реакції на фідбек
(немає заповненого каналу відгуку в Play Console). 14-денний чекліст
обнулено, старт нового вікна — 2026-08-31.

**Зроблено до 2026-09-01 (перевірено скрінами Play Console/Gmail):**

- ✅ Поле «URL- чи електронна адреса для відгуку» (Закрите тестування → Alpha
  → Тестувальники) заповнено — `mythgate5e@gmail.com`, та сама адреса, що й
  для запитів на видалення акаунта.
- ✅ 16 тестувальників активно залучені: реальні листи з конкретним фідбеком
  на `mythgate5e@gmail.com` (наприклад, «Проблеми з синхронізацією листа
  персонажа», «Неправильне відображення субменю в бестіарії «Заклинань»»,
  зі скріншотами) і відгуки безпосередньо в Play Маркеті — не лише opted-in
  без активності, яка й була причиною відмови.
- ✅ **Перший видимий цикл «фідбек → реакція» задокументовано**: на лист
  «Неправильне відображення субменю в бестіарії» власник продукту відповів
  того ж дня (31 серп.) — «Вже додано додатковий фільтр. Дякую за
  уважність!»; на інший — «Фільтри будуть виправлені та доповнені»,
  «Дякую за відгук! Буде виправлено» (одразу, без затримки на дні).
  Це і є той видимий Google best-practice цикл, якого не вистачало в
  першому вікні.
- ⏳ Продовжувати тримати активність усіх 16 тестувальників рівномірно
  протягом усіх 14 днів (не лише перший день) — Play Console dashboard
  показує лічильник «зареєстровані протягом N днів», звірятись з ним
  періодично.

Деталі, конкретна знахідка (порожнє поле фідбеку) і повний план дій —
`docs/release-blockers.md` п.1. Орієнтовний фініш вікна — **~2026-09-14**,
повторна заявка одразу після, розгляд ще до 7 днів. R5-1 (регресійний
чекліст) і R5-2 (фінальний AAB) готуємо паралельно з цим вікном — та сама
логіка, що й з першим 14-денним відліком: очікування Google не означає
простій.

**Свідомий виняток зі scope freeze (рішення власника продукту 2026-09-01,
`CLAUDE.md` Виняток 3):** протягом цього ж вікна також закриваємо COL-4
повністю (семантичний merge лічильників) і COL-6+COL-9 (presence + журнал
у підколекцію) — раніше «після 1.0», тепер R5-0a/R5-0b нижче. Деталі й
цільовий дизайн — `docs/collaborative-editing.md` §3.1.

| #    | Задача                                                                                                                   | DoD                                                               |
| ---- | ------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------- |
| R5-0 | ✅ **Значною мірою зроблено.** Усунути причини відмови: заповнити канал фідбеку тестувальників, повторно активно залучити 16 тестувальників на нові 14 днів, задокументувати хоча б один цикл «фідбек → реліз» | Поле відгуку заповнене — ✅; тестувальники активно відкривають застосунок протягом усіх 14 днів — ⏳ триває, перший день підтверджено; хоча б один цикл «фідбек → видима реакція» — ✅ задокументовано (див. вище) |
| R5-0a | ✅ **Зроблено** (коміт `8a312c1`). COL-4 повністю: `FieldValue.increment()` для HP/slots/ресурсів, `arrayUnion`/`arrayRemove` для conditions — `docs/collaborative-editing.md` §3.1 | Одночасні дельти двох клієнтів на HP/ресурсі не конфліктують, фінальне значення коректне на обох; тести на двох клієнтах — ⏳ контрольований прогін ще не зроблено |
| R5-0b | ⏳ **Код зроблено 2026-09-05, правила задеплоєні в prod, ще не закомічено в git.** COL-6+COL-9: presence з heartbeat + `changeHistory[]` → підколекція `characterSheets/{id}/changes` — `docs/collaborative-editing.md` §3.1 | Presence видно на другому клієнті — ✅ підтверджено через Firestore Console; журнал пишеться в підколекцію, документ персонажа більше не росте від історії — ✅ підтверджено; повний контрольований тест одночасних дельт HP/conditions двома клієнтами — ⏳ не зроблено; новий production AAB — ⏳ не зроблено |
...
| R5-3 | Повторна заявка на production access після завершення нового 14-денного вікна                                            | Заявка подана вдруге, дата зафіксована                            |
| R5-4 | Submit у production зі **staged rollout 10%** (після схвалення заявки)                                                   | Реліз відправлений                                                |
| R5-5 | Спостереження Crashlytics/Analytics 48–72 год: crash-free rate, частота `conflict_shown` і `permission_denied_on_upload` | Метрики зібрані                                                   |
| R5-6 | Розкатка 10% → 50% → 100%, JS-хотфікси через EAS Update                                                                  | 1.0.0 у 100%                                                      |

---

## 3. Що навмисно НЕ входить у 1.0

Повний зведений список — **`docs/post-1.0-backlog.md`** (винесено окремо
2026-08-20, щоб не дублювати той самий перелік тут, у `docs/audit-2026-07.md`
§7 і в `docs/release-blockers.md`). Коротко: Expo SDK 57, семантичний merge
лічильників (COL-4), presence + журнал у підколекцію, розпил
`useCharacterActions.tsx`, CI/E2E (свідоме рішення НЕ робити, не «пізніше»),
GM Desktop Workspace, Picture-in-Picture.
