...
2. **Семантичний merge лічильників (COL-4, повністю).** `FieldValue.increment()` для
   HP/slots/ресурсів і `arrayUnion` для conditions — зараз секція `combat` мержиться
   цілком, тож DM, що крутить HP, і гравець, що ставить condition, отримують конфлікт
   на всю секцію (CLAUDE.md §5.5). Дроблення секцій — продуктове рішення з тестами,
   не точковий фікс.

3. **Справжній presence (COL-6) + журнал змін у підколекцію (COL-9).** Presence з
   heartbeat («хто щойно правив» у шапці) і винесення `changeHistory[]` з масиву в
   документі в окрему підколекцію — для масштабованості спільного редагування поза
   рамками MVP-моделі `docs/collaborative-editing.md`.
