...
1. ~~⏳ **R1-1 — 14-денний відлік у процесі**~~ — ✅ **Вікно закрито, заявку
   подано (перевірено 2026-08-28, скрін «Інформаційна панель» власника
   продукту).** Усі три пункти чекліста «Подання заявки на доступ до
   робочої версії» виконані: ~~«Опублікувати версію для закритого
   тестування»~~ ✅, ~~«Залучити принаймні 12 користувачів до закритого
   тестування»~~ ✅, ~~«Провести закрите тестування тривалістю принаймні
   14 днів зі щонайменше 12 користувачами»~~ ✅ — вікно (старт 2026-08-12)
   завершилось 2026-08-26, усі 12 тестувальників лишались opted-in
   безперервно. Кнопка «Подати заявку» стала активною, **заявку на доступ
   до робочої версії подано 2026-08-27 (четвер, 20:24)**. Скрін
   «Інформаційна панель» підтверджує: розділ «Робоча версія» → «Неактивний»
   (очікуваний стан до рішення), нижче — «Ми отримали вашу заявку на доступ
   до робочої версії... Зазвичай на це потрібно до 7 днів, але іноді може
   знадобитися більше часу».

   **Новий критичний шлях:** очікування рішення Google по заявці,
   орієнтовно до **2026-09-03** (7 днів від подання, за практикою може
   затягнутися). Код тут нічого не прискорює. Паралельно з очікуванням має
   сенс пройти R5 (`docs/release-plan-google-play.md` §2, регресійний
   чекліст §4, підготовка фінального production AAB) — саме так, як і
   попередній 14-денний відлік, це не простій.
