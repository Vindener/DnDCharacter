...
export async function readUsersByUids(uids = []) {
  ...
  const snaps = await Promise.all(uniq.map((uid) => getDoc(doc(db, 'users', String(uid)))));
...
export async function deletePersonalDoc(docId) {
  ...
  try {
    const { deleteDoc: modularDeleteDoc } = await import('@react-native-firebase/firestore');
