  ...
  return {
    ...
    FlatList: ({
      ...
      ReactNativeMock.createElement(
        ...
        data.length ? data.map((item, index) => ReactNativeMock.createElement('Item', { key: item.id }, renderItem({ item, index }))) : ListEmptyComponent,
      ),
  };
});
...
vi.mock('@/shared/components/MonsterCard/MonsterCard', () => ({
  MonsterCard: ({
    ...
    React.createElement(
      ...
      React.createElement('Pressable', { testID: 'monsterCard.addToEncounterButton', onPress: (event: { stopPropagation: () => void }) => {
        event.stopPropagation();
        onAddToEncounter?.(monster);
      } }),
      React.createElement('Pressable', { testID: 'monsterCard.pinButton', onPress: (event: { stopPropagation: () => void }) => {
        event.stopPropagation();
        onTogglePin?.(monster.id);
      } }),
      React.createElement('Pressable', { testID: 'monsterCard.favoriteButton', onPress: (event: { stopPropagation: () => void }) => {
        event.stopPropagation();
...
async function renderBestiary(): Promise<ReactTestRenderer> {
  let tree: ReactTestRenderer;
  await act(async () => {
    tree = create(<Bestiary />);
  });
  return tree!;
}
