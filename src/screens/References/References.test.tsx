...
describe('References screen', () => {
  ...
  it('renders every localized rules reference without source badges', () => {
    ...
    ['conditions', 'actions-in-combat', 'resting', 'ability-checks', 'saving-throws', 'equipment', 'spellcasting-basics'].forEach(
      (entryId) => {
        expect(
          tree.root.findByProps({ testID: `references.srd.${entryId}` }).findAllByProps({ name: 'chevron-forward-outline' }),
        ).toHaveLength(0);
      },
    );
